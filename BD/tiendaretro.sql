...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 21-10-2021 a las 18:24:48
...
INSERT INTO `console` (`id_console`, `name`, `made_in`) VALUES
(1, 'Nintendo', 'Japon'),
(2, 'Super Nintendo', 'Japon'),
(3, 'Master System', 'Japon'),
(4, 'Sega Megadrive', 'Japon');
...
(4, 'Metal Gear', 1988, 'https://static.wikia.nocookie.net/metalgear/images/4/40/Metal_Gear_Portada.jpg/revision/latest/scale-to-width-down/742?cb=20130102083410&path-prefix=es', 'Primer título de la saga de Hideo Kojima. Infiltración en territorio enemigo con Solid Snake... y todo el carisma que esperas de un MG.', 1, '36', 50),
...
(7, 'Bubble Bobble', 1990, 'https://static.wikia.nocookie.net/nintendo/images/b/b6/BubbleBobble_NES_NA.jpg/revision/latest/scale-to-width-down/400?cb=20160718154503&path-prefix=es', 'Una de las melodías más reconocibles para uno de los arcades más divertidos y versionados de siempre. En cooperativo es genial.', 1, '19', 50),
...
CREATE TABLE `user` (
  ...
  `name` varchar(50) DEFAULT NULL,
  `subname` varchar(70) DEFAULT NULL,
...
ALTER TABLE `order_line`
  ADD PRIMARY KEY (`id_order_line`,`id_order`),
  ADD KEY `order_line_fk0` (`id_order`),
  ADD KEY `order_line_fk1` (`id_game`);
...
--
ALTER TABLE `order`
  ADD CONSTRAINT `Order_fk0` FOREIGN KEY (`id_order`) REFERENCES `order_line` (`id_order_line`),
  ADD CONSTRAINT `Order_fk1` FOREIGN KEY (`id_user`) REFERENCES `user` (`id_user`);

--
-- Filtros para la tabla `order_line`
--
ALTER TABLE `order_line`
  ADD CONSTRAINT `order_line_fk0` FOREIGN KEY (`id_order`) REFERENCES `order` (`id_order`),
  ADD CONSTRAINT `order_line_fk1` FOREIGN KEY (`id_game`) REFERENCES `game` (`id_game`);
COMMIT;
